 ...
 *   Servo sweep times increased for slower servos.
 *   Adjusted turret microsecond range to 620...1520...2420
 *   v0.83
 */
...
int findOpening()
{
  ...
  do
  {
    ...
    if(k == -1)                                      // Change inc/dec value if limit reached
    {
      k = ki;
      inc = -inc;
      dt = 350;                                      // Turret will take longer to get there
    }
    if(k == 11)
    {
      k = ki;
    ...
    dt = 150;                                        // Reset for small increment turret movement
